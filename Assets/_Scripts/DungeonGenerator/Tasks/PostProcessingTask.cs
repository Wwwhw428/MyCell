...
{
    [CreateAssetMenu(menuName = "MyCell/DungeonGenerator/Post-processing", fileName = "Post processing")]
    public class PostProcessingTask : DungeonGeneratorPostProcessingGrid2D
    {
        ...
        private void SetupLevelMap(DungeonGeneratorLevelGrid2D level)
        {
            ...
            if (!Application.isPlaying || !GeneratorManager.Instance.LevelMapSupported())
            {
                return;
            }
            ...
            var mapLayer = LayerMask.NameToLayer(GeneratorManager.LevelMapLayer);
        ...
        {
            var environmentLayer = LayerMask.NameToLayer(GeneratorManager.StaticEnvironmentLayer);
            ...
            // motor.staticEnvLayerMask = LayerMask.GetMask(GeneratorManager.StaticEnvironmentLayer);
        }
        ...
        private void SetSpawnPosition(DungeonGeneratorLevelGrid2D level)
        {
            ...
            var entranceRoomInstance = level
                .RoomInstances
                .FirstOrDefault(x => ((MyRoom) x.Room).Type == MyRoomType.Entrance);
